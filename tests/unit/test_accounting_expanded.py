...
@pytest.mark.parametrize("price,quantity,fee", [("0.25", 1, "0.01"), ("0.40", 3, "0.02"), ("0.70", 5, "0.03")])
def test_buy_cash_change_equals_notional_plus_fee(
    ...
    updated = apply_fill_to_portfolio(portfolio, fill("f", outcome, OrderIntent.BUY, price, quantity, fee))
    assert updated.cash == Decimal("100") - Decimal(price) * Decimal(quantity) - Decimal(fee)


@pytest.mark.parametrize("sell_price,expected", [("0.35", Decimal("0.24")), ("0.55", Decimal("1.24"))])
...
def test_settlement_pays_winner_and_expires_loser() -> None:
    portfolio = PortfolioState(cash=Decimal("100"))
    portfolio = apply_fill_to_portfolio(portfolio, fill("yes", OutcomeSide.YES, OrderIntent.BUY, "0.40", 2))
    portfolio = apply_fill_to_portfolio(portfolio, fill("no", OutcomeSide.NO, OrderIntent.BUY, "0.30", 1))
